...
int main(int argc, char* argv[]){
  ...
  const int plotStep=atoi(argv[2]);
  ...
  double sigSMEvents[nMasses][nCats]={0.0};
  RooRealVar *mu[nMasses];
  ...
  for (int mMC=110; mMC<=150; mMC+=5){
    ...
    mu[mIt] = new RooRealVar(Form("mu_m%d",mMC),Form("mu_m%d",mMC),0.,-10,10);
  ...
  // ---- Get mass distributions  and entries ------
   ...
   // ----- def s and b yields and construct s+b model
      ...
      sigYield[mIt][cat] = new RooFormulaVar(Form("sigYield_m%d_cat%d",mMC,cat),Form("sigYield_m%d_cat%d",mMC,cat),"@0*@1",RooArgList(RooConst(sigSMEvents[mIt][cat]),*mu[mIt]));
   /*
  ...
  cout << "--- Data fitted in each category. Sig MC distributions obtained ---" << endl;
