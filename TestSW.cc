...
int main(int argc, char* argv[]){
  ...
  TFile *inFile;
  inFile = new TFile("CMS-HGG_4686pb.root");
  gROOT->SetStyle("Plain");
  gROOT->ForceStyle();
  ...
  mass->setRange(mL-10,mL+10);
  

  system("mkdir TestSWResults");
  ...
  TFile *outFile = new TFile(Form("TestSWResults/biasCheck_g%s_f%s_j%d.root",genName.c_str(),fitName.c_str(),jobNumb),"RECREATE");
  ...
  for (int cat=0; cat<nCats; cat++){
    ...
    // --- polynomials ---
    if (fitType=="pol"){
      for (int par=0; par<nFitPar; par++) fitPars[par][cat] = new RooRealVar(Form("polF_p%d_cat%d",par,cat),Form("polF_p%d_cat%d",par,cat),0.0,-2.,2.); 
  ...
  mass->setRange("testSW",mL-10,mL+10);
  RooDataSet *temp = (RooDataSet*)dataWS->data(Form("data_mass_cat%d",cat));
  data[cat] = (RooDataSet*)temp->reduce(CutRange("testSW")); 
  TH1F *dataH = (TH1F*)inFile->Get(Form("th1f_data_mass_cat%d",cat));
  mIt=0;
  for (int mMC=lowM; mMC<=highM; mMC+=5){
    if (mMC==145) continue;
    int bin = (mMC-109);
    dataSWEvents[mIt][cat] = dataH->Integral(bin,bin+20);
    cout << "mIt: " << mIt << "m: " << dataH->GetBinLowEdge(bin) << "-" << dataH->GetBinLowEdge(bin+20) << endl;
    mIt++;
  }
  ...
  // ----- draw fit to data ---------------
    ...
    text->Draw("same");
    c->Print(Form("SWplots/data/fitTodat_%s_cat%d.png",genName.c_str(),cat),"png");
    //delete c;
    ...
    for (int mMC=lowM; mMC<=highM; mMC+=5){
      ...
      for (int win=0; win<nWinds; win++){
        ...
        // ---- make legend -----
            ...
            leg->Draw("same");
            text->Draw("same");
            c1->Print(Form("SWplots/toys/fitTogen_g%s_f%s_m%d_w%s_cat%d_toy%d.png",genName.c_str(),fitName.c_str(),mMC,winName[win].c_str(),cat,itToy),"png");
